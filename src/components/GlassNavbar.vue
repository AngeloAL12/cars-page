...
const { navContainer, indicatorLeft, indicatorRight, updateIndicator } = useNavIndicator()
const { isOpen, toggle, close } = useMobileMenu()

const navItems = [
  { name: 'McQueen', to: '/' },
  { name: 'King', to: '/king' },
  { name: 'Chiks', to: '/chicks' },
  { name: 'Mater', to: '/mater' },
...
<template>
  <nav
    class="fixed top-0 left-0 right-0 z-50 md:top-4 md:left-1/2 md:right-auto md:w-fit md:-translate-x-1/2"
  >
    <div
      class="glass-effect flex items-center justify-between px-4 py-3 md:w-fit md:px-2 md:py-2 md:rounded-2xl md:justify-center"
    >
      <div class="glass-gradient hidden md:block" />

      <RouterLink to="/" class="flex items-center gap-2 md:hidden z-10" @click="close">
    ...
    <Transition name="dropdown">
      <div v-if="isOpen" class="fixed inset-0 top-15 glass-effect md:hidden">
